...
# Database Structure updated to track usernames
users = load_db("users.json")
keys = load_db("keys.json") # Stores: {"key": {"duration": sec, "used_by": username/None}}
...
def start(message):
    ...
    # Update user database with latest username
    if uid not in users:
...
# --- STEALTH ADMIN COMMANDS (Upgraded) ---
...
def remove_key(message):
    ...
    except: bot.reply_to(message, "`/remove_key <KEY>`")
...
def redeem(message):
    ...
    try:
        ...
        if k in keys and keys[k]['used_by'] is None:
            ...
            keys[k]['used_by'] = uname # Mark key as used by this user
            save_db("users.json", users)
            save_db("keys.json", keys)
            bot.reply_to(message, "👑 **VIP GRANTED**")
        else: bot.reply_to(message, "❌ Key Invalid or Already Used.")
...
print("SASTA DEVELOPER v9 MASTER DATABASE ONLINE...")
